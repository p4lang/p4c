...
void InspectPnaProgram::postorder(const IR::P4Parser *p) {
    if (pinfo->block_type.count(p)) {
        ...
        if (info == MAIN_PARSER)
            pinfo->parsers.emplace("main_parser", p);
    }
}
...
void InspectPnaProgram::postorder(const IR::P4Control *c) {
    if (pinfo->block_type.count(c)) {
        ...
        if (info == MAIN_CONTROL)
            pinfo->pipelines.emplace("main_control", c);
        // else if (info == PRE_CONTROL)
        //     pinfo->pipelines.emplace("pre_control", c);
        else if (info == MAIN_DEPARSER)
            pinfo->deparsers.emplace("main_deparser", c);
    }
}
...
bool ParsePnaArchitecture::preorder(const IR::PackageBlock *block) {
    auto pkg = block->findParameterValue("main_parser");
    if (pkg == nullptr) {
    ...
    // structure->parsers.emplace("main_parser", main_parser->container);

    // pkg = block->findParameterValue("pre_control");
    // if (pkg == nullptr) {
    //     modelError("Package %1% has no parameter named 'pre_control'", block);
    //     return false;
    // }
    // auto pre_control = pkg->to<IR::ControlBlock>();
    // if (pre_control == nullptr) {
    //     modelError("%1%: 'pre_control' argument should be in bound", block);
    //     return false;
    // }
    // // structure->pipelines.emplace("pre_control", pre_control->container);

    pkg = block->findParameterValue("main_control");
    if (pkg == nullptr) {
    ...
    // structure->pipelines.emplace("main_control", main_control->container);

    pkg = block->findParameterValue("main_deparser");
    if (pkg == nullptr) {
    ...
    // structure->deparsers.emplace("main_deparser", deparser->container);
    
    structure->block_type.emplace(main_parser->container, MAIN_PARSER);
    // structure->block_type.emplace(pre_control->container, PRE_CONTROL);
    ...
    // structure->pipeline_controls.emplace(pre_control->container->name);
