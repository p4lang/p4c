...
void PnaCodeGenerator::createStructLike(ConversionContext *ctxt, const IR::Type_StructLike *st) {
    CHECK_NULL(st);
...
void PnaCodeGenerator::createTypes(ConversionContext *ctxt) {
...
void PnaCodeGenerator::createScalars(ConversionContext *ctxt) {
...
void PnaCodeGenerator::createHeaders(ConversionContext *ctxt) {
    for (auto kv : headers) {
...
void PnaCodeGenerator::createParsers(ConversionContext *ctxt) {
    {
        auto cvt = new ParserConverter(ctxt, "main_parser");
        auto main_control = parsers.at("main_parser");
        main_control->apply(*cvt);
    }
}

void PnaCodeGenerator::createExterns() {
    /* TODO */
...
void PnaCodeGenerator::createActions(ConversionContext *ctxt) {
...
void PnaCodeGenerator::createControls(ConversionContext *ctxt) {
    auto cvt = new BMV2::ControlConverter<Standard::Arch::PNA>(ctxt, "main_control", true);
    auto main_control = pipelines.at("main_control");
    main_control->apply(*cvt);
}

void PnaCodeGenerator::createDeparsers(ConversionContext *ctxt) {
    {
        auto cvt = new DeparserConverter(ctxt, "main_deparser");
        auto main_control = deparsers.at("main_deparser");
        main_control->apply(*cvt);
    }
}
...
void PnaNicBackend::convert(const IR::ToplevelBlock *tlb) {
    CHECK_NULL(tlb);
    PnaCodeGenerator structure(refMap, typeMap);
...
cstring PnaCodeGenerator::convertHashAlgorithm(cstring algo) {
    cstring result;
...
void ExternConverter_InternetChecksum::convertExternInstance(UNUSED ConversionContext *ctxt,
    ...
    auto mainParser = pnaStructure->parsers.at("main_parser")->controlPlaneName();
    auto mainDeparser = pnaStructure->deparsers.at("main_deparser")->controlPlaneName();
...
void ExternConverter_Counter::convertExternInstance(UNUSED ConversionContext *ctxt,
    ...
    // second argument is the counter type, this is pna metadata, the converter
...
void ExternConverter_DirectCounter::convertExternInstance(UNUSED ConversionContext *ctxt,
    ...
    if (it == ctxt->structure->directCounterMap.end()) {
        ...
        // Direct Counter only has a single argument, which is pna metadata
