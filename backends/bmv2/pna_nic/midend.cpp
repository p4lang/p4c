...
The policy is: convert all enums that are not part of the pna.
...
PnaNicMidEnd::PnaNicMidEnd(CompilerOptions &options, std::ostream *outStream)
    : MidEnd(options) {
    auto convertEnums = new P4::ConvertEnums(&refMap, &typeMap, new PnaEnumOn32Bits("pna.p4"));
    ...
    if (BMV2::PnaNicContext::get().options().loadIRFromJson == false) {
        addPasses({
            ...
            new P4::ValidateTableProperties({"pna_implementation", "pna_direct_counter",
                                             "pna_direct_meter", "pna_idle_timeout", "size"}),
        ...
        auto fillEnumMap = new P4::FillEnumMap(new PnaEnumOn32Bits("pna.p4"), &typeMap);
        addPasses({
