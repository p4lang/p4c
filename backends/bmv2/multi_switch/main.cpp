...
int main(int argc, char *const argv[]) {
    for(int i = 1; i < 5; i++) {
        ...
        if (options.loadIRFromJson == false) {
            ...
            std::cout << "Look: " << jsonFileLoader.json;
        ...
        if (!options.outputFile.isNullOrEmpty()) {
            std::string output_name = pipe_name + ".json";
            cstring output(output_name);

            std::ostream *out = openFile(output, false);
            if (out != nullptr) {
