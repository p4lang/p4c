...
template <Standard::Arch arch> class ControlConverter : public Inspector {
...
protected:
  ...
  convertTable(const CFG::TableNode *node, Util::JsonArray *action_profiles,
      ...
      for (auto ke : key->keyElements) {
        ...
        if (!ket->is<IR::Type_Bits>() && !ket->is<IR::Type_Boolean>())
          ::error(ErrorType::ERR_UNSUPPORTED,
                  ...
                  "Supported key types are be bit<> or boolean.",
                  expr, ket);
        ...
        // 2) if there is at least one TERNARY or OPTIONAL field, then the table
        // is TERNARY 3) if there is a LPM field, then the table is LPM 4)
        // otherwise the table is EXACT
        ...
        if (match_type != table_match_type) {
          ...
          if ((match_type == corelib.ternaryMatch.name ||
               match_type ==
                   BMV2::MatchImplementation::optionalMatchTypeName) &&
              table_match_type != BMV2::MatchImplementation::rangeMatchTypeName)
        ...
        if (mask != 0)
          keyelement->emplace(
              "mask", stringRepr(mask, ROUNDUP(expr->type->width_bits(), 8)));
        else
    ...
    if (ctrs != nullptr) {
      // The counters attribute should list the counters of the table, accessed
      // in actions of the table.  We should be checking that this attribute and
    ...
    for (auto a : al->actionList) {
      if (a->expression->is<IR::MethodCallExpression>()) {
        ...
        if (mce->arguments->size() > 0)
          ::error(ErrorType::ERR_UNSUPPORTED,
                  "%1%: actions in action list with arguments not supported",
                  a);
      }
    ...
    auto defact = table->properties->getProperty(
        IR::TableProperties::defaultActionPropertyName);
    if (defact != nullptr) {
      if (!simple) {
        ::warning(ErrorType::WARN_UNSUPPORTED,
                  "Target does not support default_action for %1% (due to "
                  "action profiles)",
                  table);
        return result;
      }
      ...
      if (expr->is<IR::PathExpression>()) {
        ...
        BUG_CHECK(decl->is<IR::P4Action>(), "%1%: should be an action name",
                  expr);
      ...
      BUG_CHECK(actionid != INVALID_ACTION_ID, "Could not find id for %1%",
                action);
  ...
  void convertTableEntries(const IR::P4Table *table,
    ...
    for (auto e : entriesList->entries) {
      ...
      for (auto k : keyset->components) {
        ...
        if (matchType == corelib.exactMatch.name) {
          if (k->is<IR::Constant>())
            ...
            key->emplace(
                "key", stringRepr(k->to<IR::BoolLiteral>()->value ? 1 : 0, k8));
          else
            ::error(ErrorType::ERR_UNSUPPORTED,
          ...
          if (k->is<IR::Mask>()) {
            ...
            key->emplace("key",
                         stringRepr(km->left->to<IR::Constant>()->value, k8));
            key->emplace("mask",
          ...
          if (k->is<IR::Mask>()) {
            ...
            auto mask = static_cast<unsigned long>(
      ...
      if (!actionRef->is<IR::MethodCallExpression>())
        ::error(ErrorType::ERR_INVALID, "Invalid action '%1%' in entries list.",
                actionRef);
  ...
  cstring getKeyMatchType(const IR::KeyElement *ke) {
    ...
    auto mt =
        ctxt->refMap->getDeclaration(path, true)->to<IR::Declaration_ID>();
  ...
  bool handleTableImplementation(const IR::Property *implementation,
                                 const IR::Key *key, Util::JsonObject *table,
    ...
    if (propv->expression->is<IR::ConstructorCallExpression>()) {
      auto cc = P4::ConstructorCall::resolve(
          propv->expression->to<IR::ConstructorCallExpression>(), ctxt->refMap,
          ctxt->typeMap);
      ...
      apname = implementation->controlPlaneName(
          ctxt->refMap->newName("action_profile"));
      ...
      action_profile->emplace_non_null("source_info",
      ...
      auto actionSelectorName =
          Standard::ActionSelectorTraits<arch>::typeName();
      if (implementationType->name == actionSelectorName) {
        BUG_CHECK(arguments->size() == 3, "%1%: expected 3 arguments",
                  arguments);
        isSimpleTable = false;
        ...
        ::error(ErrorType::ERR_UNEXPECTED, "%1%: expected value for property",
                propv);
      }
      ...
      ::error(ErrorType::ERR_UNEXPECTED, "%1%: unexpected value for property",
              propv);
      return false;
    }
...
public:
  ...
  bool preorder(const IR::P4Control *cont) override {
    ...
    for (auto c : cont->controlLocals) {
      if (c->is<IR::Declaration_Constant>() ||
          c->is<IR::Declaration_Variable>() || c->is<IR::P4Action>() ||
          c->is<IR::P4Table>())
        continue;
  ...
  explicit ControlConverter(ConversionContext *ctxt, cstring name,
