...
//TODO templatize this class. No difference from PSA BFRuntimeArchHandler 
class BFRuntimeArchHandlerPNA final : public P4RuntimeArchHandlerCommon<Arch::PNA> {
 ...
 public:
    ...
    using ArchCounterExtern = CounterExtern<Arch::PNA>;
    using CounterTraits = Helpers::CounterlikeTraits<ArchCounterExtern>;
    using ArchMeterExtern = MeterExtern<Arch::PNA>;
    ...
    BFRuntimeArchHandlerPNA(ReferenceMap* refMap, TypeMap* typeMap,
        ...
        : P4RuntimeArchHandlerCommon<Arch::PNA>(refMap, typeMap, evaluatedProgram) {
    ...
    void collectExternInstance(P4RuntimeSymbolTableIface* symbols,
        ...
        P4RuntimeArchHandlerCommon<Arch::PNA>::collectExternInstance(symbols, externBlock);

        auto decl = externBlock->node->to<IR::IDeclaration>();
        if (decl == nullptr) return;
        if (externBlock->type->name == "Digest") {
            symbols->add(SymbolType::DIGEST(), decl);
         } else if (externBlock->type->name == ActionSelectorTraits<Arch::PNA>::typeName()) {
    ...
    void addTableProperties(const P4RuntimeSymbolTableIface& symbols,
        ...
        P4RuntimeArchHandlerCommon<Arch::PNA>::addTableProperties(
    ...
    void addExternInstance(const P4RuntimeSymbolTableIface& symbols,
                           p4configv1::P4Info* p4info,
                           const IR::ExternBlock* externBlock) override {
        P4RuntimeArchHandlerCommon<Arch::PNA>::addExternInstance(
