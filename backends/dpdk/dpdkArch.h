...
/// calculation will be unrolled in a dedicated pass.
...
/// For dpdk asm, there is no object-oriented approach. Therefore, we cannot define a
/// checksum in dpdk asm. Instead, dpdk asm only provides ckadd (checksum add) and
/// cksub (checksum sub) operations. Thus, we need to define an explicit state for each checksum
/// declaration. Essentially, this state will be declared in the header struct and
/// initialized to 0. For cksum.add(x), it will be translated to ckadd state x.
/// For dst = cksum.get(), it will be translated to mov dst state. This pass
/// collects checksum instances and indexes them.
...
/// This pass injects checksum states into the header. The decision to inject
/// state into the header instead of metadata is due to implementation details on the dpdk side,
/// specifically a question related to endianness.
...
/// This pass prepares logical expressions for subsequent branching statement optimization.
/// It breaks down expressions with parentheses such as (a && b) && c into a simpler form,
/// resulting in expressions like a && b && c. Note that the AST structure changes accordingly.
