...
const IR::Node *ConvertBinaryOperationTo2Params::postorder(IR::AssignmentStatement *a){
    ...
    if(auto r = right->to<IR::Operation_Binary>()){
        if(not isSimpleExpression(r->right) or not isSimpleExpression(r->left))
            BUG("Statement Unroll pass failed");
        if(left->equiv(*r->left)) return a;
        ...
        for(auto kv: locals_map){
            for(auto d: kv.second){
                if(auto dv = d->to<IR::Declaration_Variable>()){
                    s->fields.push_back(new IR::StructField(IR::ID(kv.first + "_" + dv->name.name), dv->type));
                }
                else if(not d->is<IR::P4Action>() and not d->is<IR::P4Table>() and not d->is<IR::Declaration_Instance>()){
...
const IR::Node *CollectLocalVariableToMetadata::postorder(IR::PathExpression* p){
    if(auto decl = refMap->getDeclaration(p->path)->to<IR::Declaration_Variable>()){
        for(auto kv: locals_map){
            for(auto d: kv.second){
                if(d->equiv(*decl)){
                    return new IR::Member(new IR::PathExpression(IR::ID("m")), IR::ID(kv.first + "_" + decl->name.name));
                }
            }
        }
...
const IR::Node *CollectLocalVariableToMetadata::postorder(IR::P4Control * c){
    ...
    for(auto d: c->controlLocals){
        if(d->is<IR::Declaration_Instance>() or d->is<IR::P4Action>() or d->is<IR::P4Table>())
            decls.push_back(d);
        else if(not d->is<IR::Declaration_Variable>()){
                std::cerr << d->node_type_name() << std::endl;
...
const IR::Node *PrependHDotToActionArgs::postorder(IR::P4Action* a){
    if(a->parameters->size() > 0){
        ...
        new_l->push_back(new IR::Parameter(IR::ID("t"), IR::Direction::None, new IR::Type_Name(IR::ID(a->name.name + "_arg_t"))));
                    ...
                    if(kv.first->to<IR::P4Control>()->name == control->name){
                        for(auto kv: args_struct_map){
                            ...
                            for(auto field: *kv.second){
                                fields->push_back(new IR::StructField(field->name, field->type));
                            }
