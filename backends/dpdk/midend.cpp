...
PnaNicMidEnd::PnaNicMidEnd(CompilerOptions &options)
    : MidEnd(options) {
    auto convertEnums =
        new P4::ConvertEnums(&refMap, &typeMap, new PsaEnumOn32Bits("pna.p4"));
    auto evaluator = new P4::EvaluatorPass(&refMap, &typeMap);
    addPasses({
        options.ndebug ? new P4::RemoveAssertAssume(&refMap, &typeMap)
                       : nullptr,
        ...
        new P4::MoveDeclarations(),
        new P4::ValidateTableProperties(
            {"pna_implementation", "pna_direct_counter", "pna_direct_meter",
             "pna_idle_timeout", "size"}),
        new P4::SimplifyControlFlow(&refMap, &typeMap),
        new P4::CompileTimeOperations(),
        new P4::TableHit(&refMap, &typeMap),
        new P4::MoveActionsToTables(&refMap, &typeMap),
