...
bool ConvertToDpdkControl::preorder(const IR::P4Action *a) {
    ...
    // remove duplicated action generated by compiler
    if (unique_actions.count(a->name.toString()) != 0)
        return false;
    unique_actions.insert(a->name.toString());

    auto action = new IR::DpdkAction(*stmt_list, a->name.toString(), *a->parameters);
