...
static bool equiv(IR::Ptr<IR::Expression> a, IR::Ptr<IR::Expression> b) {
    if (*a == *b) return true;
...
void CreateSaluInstruction::checkActions(const Util::SourceInfo &srcInfo) {
    std::map<std::pair<cstring, IR::Ptr<IR::Expression>>, IR::Ptr<IR::MAU::SaluInstruction>>
        actions_by_dest_pred;
    for (auto act : action->action) {
        ...
        std::set<IR::Ptr<IR::Expression>> preds = {pred, nullptr};
        for (auto pred : preds) {
            auto dest_and_pred = std::make_pair(dest_name, pred);
            if (actions_by_dest_pred.count(dest_and_pred)) {
                auto dest = act->to<IR::MAU::SaluInstruction>()->getOutput();
...
IR::Ptr<IR::Expression> CreateSaluInstruction::reuseCmp(const IR::MAU::SaluInstruction *cmp,
                                                        int idx) {
        ...
        if (operand) {
            if (const auto *slice = operand->to<IR::Slice>(); slice) {
                ...
                for (const auto &[assigned_param, assigned_alu] : action->output_param_to_alu) {
                    auto assigned_op = output_param_operands.at(assigned_param);
...
IR::Ptr<IR::MAU::SaluInstruction> CreateSaluInstruction::setup_output() {
...
IR::Ptr<IR::MAU::SaluInstruction> CreateSaluInstruction::createInstruction() {
        ...
        case OUTPUT:
            ...
            if (outputs.size() > size_t(output_index) && outputs[output_index] &&
                ...
                if (canOutputDirectly(operands.at(0))) {
                    ...
                    IR::Ptr<IR::MAU::SaluReg> new_operand =
