...
class P4EbpfTest(BaseTest):
    ...
    def add_port(self, dev):
        ...
        if dev.startswith("eth") and self.is_xdp_test():
            self.exec_cmd("ip link set dev s1-{} xdp pinned {}/{}".format(
    ...
    def read_map(self, name, key):
        cmd = "bpftool -j map lookup pinned {}/{} key {}".format(PIPELINE_MAPS_MOUNT_PATH, name,
                                                                 key)
    ...
    def verify_map_entry(self, name, key, expected_value, mask=None):
        ...
        if expected_value != value:
            self.fail("Map {} key {} does not have correct value. Expected {}; got {}".format(
    ...
    def clone_session_create(self, id):
        self.exec_ns_cmd("nikss-ctl clone-session create pipe {} id {}".format(
            TEST_PIPELINE_ID, id))

    def clone_session_add_member(self, clone_session, egress_port, instance=1, cos=0):
        self.exec_ns_cmd(
            "nikss-ctl clone-session add-member pipe {} id {} egress-port {} instance {} cos {}"
            .format(TEST_PIPELINE_ID, clone_session, egress_port, instance, cos))

    def clone_session_delete(self, id):
        self.exec_ns_cmd("nikss-ctl clone-session delete pipe {} id {}".format(
            TEST_PIPELINE_ID, id))

    def multicast_group_create(self, group):
        self.exec_ns_cmd("nikss-ctl multicast-group create pipe {} id {}".format(
    ...
    def multicast_group_delete(self, group):
        self.exec_ns_cmd("nikss-ctl multicast-group delete pipe {} id {}".format(
            TEST_PIPELINE_ID, group))
        ...
        if data or counters or meters:
            ...
            if meters:
                for k, v in meters.items():
                    s = s + "meter {} {}:{} {}:{} ".format(k, v["pir"], v["pbs"], v["cir"],
                                                           v["cbs"])
        return s
    ...
    def table_verify(self,
        ...
        if action is not None:
            if action != entry["action"]["id"]:
                self.fail("Invalid action ID: expected {}, got {}".format(
                    action, entry["action"]["id"]))
        if priority is not None:
            if priority != entry["priority"]:
                self.fail("Invalid priority: expected {}, got {}".format(
                    priority, entry["priority"]))
        if data:
            action_params = entry["action"]["parameters"]
            if len(action_params) != len(data):
                self.fail("Invalid number of action parameters: expected {}, got {}".format(
                    len(data), len(action_params)))
            for k, v in enumerate(data):
                if v != int(action_params[k]["value"], 0):
                    self.fail("Invalid action parameter {} (id {}): expected {}, got {}".format(
                        action_params[k]["name"], k, v, int(action_params[k]["value"], 0)))
        if references:
    ...
    def _do_counter_verify(self, bytes, packets, entry_value, counter_type):
        ...
        if expected_type != counter_type:
            self.fail("Invalid counter type, expected: \"{}\", got \"{}\"".format(
        ...
        if packets is not None:
            ...
            if counter_packets != packets:
                self.fail("Invalid counter packets, expected {}, got {}".format(
    ...
    def register_set(self, name, index=None, value=None):
        ...
        cmd = "nikss-ctl register set pipe {} {} {} {}".format(TEST_PIPELINE_ID, name, index_str,
                                                               value_str)
    ...
    def register_verify(self, name, index, expected_value):
        ...
        for idx, field_value in enumerate(reg_value.values()):
            ...
            if field_value != expected_field_value:
                self.fail("Invalid register value, expected {}, got {}".format(
