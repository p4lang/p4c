...
ExtractSuccess::ExtractSuccess(
    IR::StateVariable extractedHeader, int offset, const IR::Expression *condition,
    std::vector<std::pair<IR::StateVariable, const IR::Expression *>> fields)
    : extractedHeader(std::move(extractedHeader)),
      offset(offset),
      condition(condition),
...
void ExtractSuccess::print(std::ostream &os) const {
    ...
    os << "[ExtractSuccess] " << extractedHeader.toString() << "@" << offset
       << " | Condition: " << condition << " | Extract Size: " << extractedHeader.type->width_bits()
       << " -> ";
    for (const auto &field : fields) {
...
const IR::StateVariable &ExtractSuccess::getExtractedHeader() const { return extractedHeader; }

/* =============================================================================================
 *   ExtractFailure
 * ============================================================================================= */

ExtractFailure::ExtractFailure(IR::StateVariable extractedHeader, int offset,
                               const IR::Expression *condition)
    : extractedHeader(std::move(extractedHeader)), offset(offset), condition(condition) {}
...
Emit::Emit(IR::StateVariable emitHeader,
           std::vector<std::pair<IR::StateVariable, const IR::Expression *>> fields)
    : emitHeader(std::move(emitHeader)), fields(std::move(fields)) {}
