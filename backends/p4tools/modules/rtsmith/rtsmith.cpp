...
std::optional<RtSmithResult> runRtSmith(const CompilerResult &rtSmithResult,
    ...
    std::unique_ptr<RuntimeFuzzer> fuzzer(&RtSmithTarget::getFuzzer(*programInfo));
    ...
    if (rtSmithOptions.printToStdout()) {
        std::cout << "Generated initial configuration:\n";
        for (const auto &writeRequest : initialConfig) {
            std::cout << writeRequest->DebugString();
        }

        std::cout << "Time series updates:\n";
        for (const auto &[time, writeRequest] : timeSeriesUpdates) {
            std::cout << "Time " << time << ":\n" << writeRequest->DebugString();
        }
    }
    ...
    if (!dirPath.empty()) {
        ...
        if (rtSmithOptions.configName().has_value()) {
            initialConfigPath /= rtSmithOptions.configName().value() + "_initial_config";
        } else {
            initialConfigPath = initialConfigPath / "initial_config";
        }
        initialConfigPath += ".txtpb";
        ...
        for (size_t idx = 0; idx < timeSeriesUpdates.size(); ++idx) {
            ...
            const auto prefix = rtSmithOptions.configName().has_value()
                                    ? rtSmithOptions.configName().value() + "_"
                                    : "";
            updatePath.replace_filename(prefix + "update_" + std::to_string(idx + 1) + ".txtpb");
...
std::optional<RtSmithResult> generateConfigImpl(
    ...
    if (!P4Tools::Target::init(rtSmithOptions.target.c_str(), rtSmithOptions.arch.c_str())) {
        error("Unsupported target %1% and architecture %2%", rtSmithOptions.target,
              rtSmithOptions.arch);
        return std::nullopt;
    }
