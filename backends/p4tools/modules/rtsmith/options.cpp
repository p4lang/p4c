...
RtSmithOptions::RtSmithOptions()
    : AbstractP4cToolOptions(RtSmith::TOOL_NAME,
                             "Generate random control-plane configurations for a P4 program.") {
    registerOption(
    ...
    registerOption(
        ...
        "Specifies the control plane API to use. Must match the target: P4RUNTIME for BMv2, "
        "BFRUNTIME for Tofino.");
    registerOption(
        "--random-seed", nullptr,
        [this](const char *) {
            if (seed.has_value()) {
...
bool RtSmithOptions::validateOptions() const {
    ...
    if ((target == "bmv2" && _controlPlaneApi != "P4RUNTIME") ||
        (target == "tofino1" && _controlPlaneApi != "BFRUNTIME")) {
        error("Control-plane API %1% is not supported by target %2%", _controlPlaneApi, target);
        return false;
    }
    if (_configName &&
        (_configName->empty() || std::filesystem::path(*_configName).filename() != *_configName ||
         *_configName == "." || *_configName == "..")) {
        error("--config-name must be a nonempty file base name");
        return false;
    }
    if (!seed.has_value()) {
