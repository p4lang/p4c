...
const ProgramInfo *Bmv2V1ModelRtSmithTarget::produceProgramInfoImpl(
    ...
    if (auto path = rtSmithOptions.userP4Info()) {
        auto info = Protobuf::deserializeObjectFromFile<p4::config::v1::P4Info>(*path);
        if (!info) return nullptr;
        bmv2V1ModelProgramInfo = new Bmv2V1ModelProgramInfo(
            compilerResult, P4::P4RuntimeAPI(new p4::config::v1::P4Info(*info), nullptr));
    } else {
        bmv2V1ModelProgramInfo = new Bmv2V1ModelProgramInfo(compilerResult);
    }
