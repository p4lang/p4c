...
TEST_F(TOMLFuzzerConfigurationTest, PartialConfigPreservesDefaults) {
    auto context = SetUp("bmv2", "v1model");
    ASSERT_TRUE(context);
    RtSmith::FuzzerConfig config;
    RtSmith::TOMLUtils::overrideFuzzerConfigsInString(config, "maxTables = 0");
    EXPECT_EQ(errorCount(), 0U);
    EXPECT_EQ(config.getMaxTables(), 0);
    EXPECT_EQ(config.getMaxEntryGenCnt(), 5);
    EXPECT_EQ(config.getMaxUpdateCount(), 10U);
}

TEST_F(TOMLFuzzerConfigurationTest, RejectsInvalidValues) {
    for (const auto *input :
         {"maxTables = -1", "maxAttempts = 0", "maxEntryGenCnt = -1", "thresholdForDeletion = 101",
          "maxUpdateCount = -1", "maxTables = 4294967296", "tablesToSkip = [1]",
          "minUpdateTimeInMicroseconds = 100001", "maxTables = ["}) {
        auto context = SetUp("bmv2", "v1model");
        ASSERT_TRUE(context);
        RtSmith::FuzzerConfig config;
        RtSmith::TOMLUtils::overrideFuzzerConfigsInString(config, input);
        EXPECT_GT(errorCount(), 0U) << input;
    }
}
