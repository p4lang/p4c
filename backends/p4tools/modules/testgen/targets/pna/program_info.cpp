...
const IR::Type_Bits PnaDpdkProgramInfo::parserErrBits = IR::Type_Bits(32, false);
...
const ordered_map<cstring, const IR::Type_Declaration *>
    *PnaDpdkProgramInfo::getProgrammableBlocks() const {
...
const IR::Member *PnaDpdkProgramInfo::getTargetInputPortVar() const {
    return new IR::Member(IR::getBitType(TestgenTarget::getPortNumWidth_bits()),
                          new IR::PathExpression("*istd"), "input_port");
}

const IR::Member *PnaDpdkProgramInfo::getTargetOutputPortVar() const {
    return &PnaConstants::OUTPUT_PORT_VAR;
}

const IR::Expression *PnaDpdkProgramInfo::dropIsActive() const { return &PnaConstants::DROP_VAR; }

const IR::Expression *PnaDpdkProgramInfo::createTargetUninitialized(const IR::Type *type,
...
const IR::Type_Bits *PnaDpdkProgramInfo::getParserErrorType() const { return &parserErrBits; }

const IR::PathExpression *PnaDpdkProgramInfo::getBlockParam(cstring blockLabel,
...
const IR::Member *PnaDpdkProgramInfo::getParserParamVar(const IR::P4Parser *parser,
    ...
    if (parserApplyParams.size() > paramIndex) {
        ...
        const auto *archSpec = TestgenTarget::getArchSpec();
        structLabel = archSpec->getParamName("MainParserT", paramIndex);
    }
