...
const bit<32> VNI_MAPPING_TABLE_SIZE = 5 * 1024;
...
struct switch_header_t {
    ...
    switch_fp2_bridged_metadata_h fp2_bridged_md;
    ethernet_h                    ethernet;
    fabric_h                      fabric;
    cpu_h                         cpu;
...
control Ipv4Hash(in switch_lookup_fields_t lkp, out switch_hash_t hash) {
    @name(".ipv4_hash") Hash<bit<32>>(HashAlgorithm_t.CRC32) ipv4_hash;
...
control Ipv6Hash(in switch_lookup_fields_t lkp, out switch_hash_t hash) {
    @name(".ipv6_hash") Hash<bit<32>>(HashAlgorithm_t.CRC32) ipv6_hash;
...
control NonIpHash(in switch_header_t hdr, in switch_local_metadata_t local_md, out switch_hash_t hash) {
    @name(".non_ip_hash") Hash<bit<32>>(HashAlgorithm_t.CRC32) non_ip_hash;
...
control Lagv4Hash(in switch_lookup_fields_t lkp, out switch_hash_t hash) {
    @name(".lag_v4_hash") Hash<bit<32>>(HashAlgorithm_t.CRC32) lag_v4_hash;
    bit<32> ip_src_addr = lkp.ip_src_addr[95:64];
    bit<32> ip_dst_addr = lkp.ip_dst_addr[95:64];
    bit<8> ip_proto = lkp.ip_proto;
    bit<16> l4_dst_port = lkp.hash_l4_dst_port;
    bit<16> l4_src_port = lkp.hash_l4_src_port;
    apply {
        hash[31:0] = lag_v4_hash.get({ ip_src_addr, ip_dst_addr, ip_proto, l4_dst_port, l4_src_port });
    }
}

control Lagv6Hash(in switch_lookup_fields_t lkp, out switch_hash_t hash) {
    @name(".lag_v6_hash") Hash<bit<32>>(HashAlgorithm_t.CRC32) lag_v6_hash;
    bit<128> ip_src_addr = lkp.ip_src_addr;
    bit<128> ip_dst_addr = lkp.ip_dst_addr;
    bit<8> ip_proto = lkp.ip_proto;
    bit<16> l4_dst_port = lkp.hash_l4_dst_port;
    ...
    bit<20> ipv6_flow_label = lkp.ipv6_flow_label;
    apply {
        hash[31:0] = lag_v6_hash.get({ ipv6_flow_label, ip_src_addr, ip_dst_addr, ip_proto, l4_dst_port, l4_src_port });
    }
}
...
control InnerDtelv4Hash(in switch_header_t hdr, in switch_local_metadata_t local_md, out switch_hash_t hash) {
    @name(".inner_dtelv4_hash") Hash<bit<32>>(HashAlgorithm_t.CRC32) inner_dtelv4_hash;
    ...
    apply {
        hash[31:0] = inner_dtelv4_hash.get({ ip_src_addr, ip_dst_addr, ip_proto, l4_src_port });
    }
}

control InnerDtelv6Hash(in switch_header_t hdr, in switch_local_metadata_t local_md, out switch_hash_t hash) {
    @name(".inner_dtelv6_hash") Hash<bit<32>>(HashAlgorithm_t.CRC32) inner_dtelv6_hash;
    ...
    apply {
        hash[31:0] = inner_dtelv6_hash.get({ ipv6_flow_label, ip_src_addr, ip_dst_addr, ip_proto, l4_src_port });
    }
}
        ...
        if (!(local_md.bypass & SWITCH_INGRESS_BYPASS_STP != 0)) {
            if (multiple_stp_enable) {
                ...
                if (local_md.bd[15:12] == 0) {
                    stp_md.state_[0:0] = stp_check0.execute(hash.get({ local_md.bd[11:0], local_md.ingress_port[6:0] }));
                    stp_md.state_[1:1] = stp_check1.execute(hash.get({ local_md.bd[11:0], local_md.ingress_port[6:0] }));
                }
            }
        }
    }
...
control VlanXlate(inout switch_header_t hdr, in switch_local_metadata_t local_md)(switch_uint32_t bd_table_size, switch_uint32_t port_bd_table_size) {
...
control Fibv6(in bit<128> dst_addr, inout switch_local_metadata_t local_md)(switch_uint32_t host_table_size, switch_uint32_t host64_table_size, switch_uint32_t lpm_table_size, switch_uint32_t lpm64_table_size=1024) {
    @immediate(0) @name(".ipv6_host") table host {
        key = {
            local_md.vrf: exact @name("vrf") ;
            dst_addr    : exact @name("ip_dst_addr") ;
        }
    ...
    Alpm(number_partitions = 2048, subtrees_per_partition = 2, atcam_subset_width = 32, shift_granularity = 2) algo_lpm64;
...
control EgressVRF(inout switch_header_t hdr, inout switch_local_metadata_t local_md) {
    @name(".set_vrf_properties") action set_vrf_properties(switch_tunnel_vni_t vni, mac_addr_t smac) {
...
control Nexthop(inout switch_local_metadata_t local_md)(switch_uint32_t nexthop_table_size, switch_uint32_t ecmp_group_table_size, switch_uint32_t ecmp_selection_table_size, switch_uint32_t ecmp_max_members_per_group=64) {
    ...
    @name(".set_ecmp_properties_post_routed_flood") action set_ecmp_properties_post_routed_flood(switch_bd_t bd, switch_mgid_t mgid, switch_nexthop_t nexthop_index, switch_nat_zone_t zone) {
        local_md.nexthop = nexthop_index;
        set_nexthop_properties_post_routed_flood(bd, mgid, zone);
    }
    @name(".set_nexthop_properties_glean") action set_nexthop_properties_glean() {
        local_md.flags.glean = true;
    }
    @name(".set_ecmp_properties_glean") action set_ecmp_properties_glean(switch_nexthop_t nexthop_index) {
        local_md.nexthop = nexthop_index;
        set_nexthop_properties_glean();
    }
    @name(".set_nexthop_properties_drop") action set_nexthop_properties_drop() {
...
control IngressMirror(inout switch_header_t hdr, in switch_local_metadata_t local_md, in ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr) {
    ...
    apply {
        if (ig_intr_md_for_dprsr.mirror_type == 1) {
            mirror.emit<switch_port_mirror_metadata_h>(local_md.mirror.session_id, { local_md.mirror.src, local_md.mirror.type, local_md.timestamp, local_md.mirror.session_id });
        } else if (ig_intr_md_for_dprsr.mirror_type == 3) {
            mirror.emit<switch_dtel_drop_mirror_metadata_h>(local_md.dtel.session_id, { local_md.mirror.src, local_md.mirror.type, local_md.timestamp, local_md.dtel.session_id, local_md.lag_hash, local_md.dtel.report_type, 0, local_md.ingress_port, 0, local_md.egress_port, 0, local_md.qos.qid, local_md.drop_reason });
...
control EgressMirror(inout switch_header_t hdr, in switch_local_metadata_t local_md, in egress_intrinsic_metadata_for_deparser_t eg_intr_md_for_dprsr) {
    ...
    apply {
        if (eg_intr_md_for_dprsr.mirror_type == 1) {
            mirror.emit<switch_port_mirror_metadata_h>(local_md.mirror.session_id, { local_md.mirror.src, local_md.mirror.type, local_md.ingress_timestamp, local_md.mirror.session_id });
            ...
            mirror.emit<switch_dtel_switch_local_mirror_metadata_h>(local_md.dtel.session_id, { local_md.mirror.src, local_md.mirror.type, local_md.ingress_timestamp, local_md.dtel.session_id, local_md.dtel.hash, local_md.dtel.report_type, 0, local_md.ingress_port, 0, local_md.egress_port, 0, local_md.qos.qid, 0, local_md.qos.qdepth, local_md.egress_timestamp });
        } else if (eg_intr_md_for_dprsr.mirror_type == 3) {
            mirror.emit<switch_dtel_drop_mirror_metadata_h>(local_md.dtel.session_id, { local_md.mirror.src, local_md.mirror.type, local_md.ingress_timestamp, local_md.dtel.session_id, local_md.dtel.hash, local_md.dtel.report_type, 0, local_md.ingress_port, 0, local_md.egress_port, 0, local_md.qos.qid, local_md.drop_reason });
        } else if (eg_intr_md_for_dprsr.mirror_type == 5) {
            mirror.emit<switch_simple_mirror_metadata_h>(local_md.dtel.session_id, { local_md.mirror.src, local_md.mirror.type, local_md.dtel.session_id });
        }
    }
}
...
control MirrorRewrite(inout switch_header_t hdr, inout switch_local_metadata_t local_md, out egress_intrinsic_metadata_for_deparser_t eg_intr_md_for_dprsr)(switch_uint32_t table_size=1024) {
    ...
    action rewrite_dtel_report(mac_addr_t smac, mac_addr_t dmac, ipv4_addr_t sip, ipv4_addr_t dip, bit<8> tos, bit<8> ttl, bit<16> udp_dst_port, switch_mirror_session_t session_id, bit<16> max_pkt_len) {
        ...
        local_md.pkt_length = local_md.pkt_length + DTEL_REPORT_V0_5_OUTER_HEADERS_LENGTH;
    }
...
control EgressPortMirror(in switch_port_t port, inout switch_mirror_metadata_t mirror_md)(switch_uint32_t table_size=288) {
...
control IngressPortMapping(inout switch_header_t hdr, inout switch_local_metadata_t local_md, inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm, inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr)(switch_uint32_t port_vlan_table_size, switch_uint32_t bd_table_size, switch_uint32_t port_table_size=288, switch_uint32_t vlan_table_size=4096, switch_uint32_t double_tag_table_size=1024) {
    ...
    @name(".set_cpu_port_properties") action set_cpu_port_properties(switch_port_lag_index_t port_lag_index, switch_ig_port_lag_label_t port_lag_label, switch_yid_t exclusion_id, switch_qos_trust_mode_t trust_mode, switch_qos_group_t qos_group, switch_pkt_color_t color, switch_tc_t tc) {
...
control LAG(inout switch_local_metadata_t local_md, in switch_hash_t hash, out switch_port_t egress_port) {
    ...
    @name(".lag_table") table lag {
        ...
        // const default_action = set_lag_port(8);
...
action set_port_state_properties(inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm, inout switch_local_metadata_t local_md, switch_ig_port_lag_label_t port_lag_label, switch_yid_t exclusion_id, switch_meter_index_t meter_index, switch_sflow_id_t sflow_session_id) {
    local_md.ingress_port_lag_label = port_lag_label;
    ig_intr_md_for_tm.level2_exclusion_id = exclusion_id;
    local_md.checks.same_if = SWITCH_FLOOD;
    local_md.sflow.session_id = sflow_session_id;
}
action set_bd_state_properties(inout switch_local_metadata_t local_md, switch_bd_label_t bd_label, bool ipv4_unicast_enable, bool ipv4_multicast_enable, bool igmp_snooping_enable, bool ipv6_unicast_enable, bool ipv6_multicast_enable, bool mld_snooping_enable, bool mpls_enable, switch_packet_action_t vrf_ttl_violation, bool vrf_ttl_violation_valid, switch_packet_action_t vrf_ip_options_violation, switch_nat_zone_t zone) {
    local_md.bd_label = bd_label;
    local_md.ipv4.unicast_enable = ipv4_unicast_enable;
    local_md.ipv4.multicast_enable = ipv4_multicast_enable;
    local_md.ipv4.multicast_snooping = igmp_snooping_enable;
...
control IngressPortBDState(inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm, inout switch_local_metadata_t local_md) {
    @use_hash_action(0) table port_state {
        ...
        const default_action = set_port_state_properties(ig_intr_md_for_tm, local_md, 0, 0, 0, 0);
        size = 512;
    }
    @use_hash_action(0) table bd_state {
        ...
        const default_action = set_bd_state_properties(local_md, 0, false, false, false, false, false, false, false, 0, false, 0, 0);
        size = 8192;
    }
    apply {
        port_state.apply();
...
control TunnelNexthop(inout switch_header_t hdr, inout switch_local_metadata_t local_md) {
    @name(".l2_tunnel_encap") action l2_tunnel_encap(switch_tunnel_type_t type, switch_tunnel_ip_index_t dip_index, switch_tunnel_index_t tunnel_index) {
...
control TunnelRewrite(inout switch_header_t hdr, inout switch_local_metadata_t local_md) {
    ...
    @name(".encap_dscp_v4_pipe_mode") action encap_dscp_v4_pipe_mode(bit<6> dscp_val) {
        hdr.ipv4.diffserv[1:0] = 0;
        hdr.ipv4.diffserv[7:2] = dscp_val;
    }
    @name(".encap_dscp_v6_pipe_mode") action encap_dscp_v6_pipe_mode(bit<6> dscp_val) {
...
control TunnelReplication(in egress_intrinsic_metadata_t eg_intr_md, inout switch_local_metadata_t local_md) {
    @name(".tunnel_rid_hit") action rid_hit(switch_bd_t bd, switch_nexthop_t nexthop, switch_nexthop_t tunnel_nexthop) {
        local_md.nexthop = nexthop;
        local_md.tunnel_nexthop = tunnel_nexthop;
    }
    @name(".tunnel_rid_miss") action rid_miss() {
    }
    @name(".tunnel_rid") table rid {
        key = {
...
control L2VniMap(inout switch_header_t hdr, inout switch_local_metadata_t local_md) {
    @name(".set_l2_vni") action set_l2_vni(switch_tunnel_vni_t vni) {
        local_md.tunnel.vni = vni;
    }
...
control MulticastReplication(in egress_intrinsic_metadata_t eg_intr_md, inout switch_local_metadata_t local_md)(switch_uint32_t table_size=4096) {
    @name(".multicast_rid_hit") action rid_hit(switch_bd_t bd) {
...
control IngressQoSMap(inout switch_header_t hdr, inout switch_local_metadata_t local_md)(switch_uint32_t dscp_map_size=2048, switch_uint32_t pcp_map_size=256) {
    ...
    apply {
        if (!(local_md.bypass & SWITCH_INGRESS_BYPASS_QOS != 0) && local_md.qos.trust_mode & SWITCH_QOS_TRUST_MODE_TRUST_DSCP == SWITCH_QOS_TRUST_MODE_TRUST_DSCP && local_md.lkp.ip_type != SWITCH_IP_TYPE_NONE) {
            dscp_tc_map.apply();
        } else if (!(local_md.bypass & SWITCH_INGRESS_BYPASS_QOS != 0) && local_md.qos.trust_mode & SWITCH_QOS_TRUST_MODE_TRUST_PCP == SWITCH_QOS_TRUST_MODE_TRUST_PCP && hdr.vlan_tag[0].isValid()) {
            pcp_tc_map.apply();
        }
    }
}
...
control WRED(inout switch_header_t hdr, in switch_local_metadata_t local_md, in egress_intrinsic_metadata_t eg_intr_md, out bool wred_drop) {
...
@pa_solitary("ingress" , "local_md.flags.ipv4_checksum_err") @pa_no_overlay("ingress" , "smac_src_move") control SwitchIngress(inout switch_header_t hdr, inout switch_local_metadata_t local_md, in ingress_intrinsic_metadata_t ig_intr_md, in ingress_intrinsic_metadata_from_parser_t ig_intr_from_prsr, inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr, inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm) {
    ...
    LOU() lou;
    Fibv4(IPV4_HOST_TABLE_SIZE, IPV4_LPM_TABLE_SIZE, false) ipv4_fib;
