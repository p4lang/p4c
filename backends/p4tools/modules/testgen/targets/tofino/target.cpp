...
AbstractTofinoTestgenTarget::AbstractTofinoTestgenTarget(std::string deviceName,
                                                         std::string archName)
    : TestgenTarget(std::move(deviceName), std::move(archName)) {}
...
TofinoTestBackend *AbstractTofinoTestgenTarget::getTestBackendImpl(
    ...
    return new TofinoTestBackend(*programInfo.checkedTo<TofinoProgramInfo>(),
...
const JBayProgramInfo *JBay_T2naTestgenTarget::produceProgramInfoImpl(
    ...
    for (size_t pipeIdx = 0; pipeIdx < mainDeclArgs->size(); ++pipeIdx) {
        ...
        BUG_CHECK(blocks.size() == 6 || blocks.size() == 7,
                  "%1%: Not a valid pipeline instantiation.", pipe);
