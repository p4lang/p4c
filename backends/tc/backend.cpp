...
bool ScanWidths::big_x_small_mul(const IR::Expression *big, const IR::Constant *small) {
...
static void gen_addsat(EBPF::CodeBuilder *bld, const WidthRec *wr) {
    ...
    } else {  // really need only u9, but can't count on that existing, ugh
        // (for that matter, can't count on u16 existing except pragmatically)
        bld->append(" u16 a;\n");
        bld->append("\n");
        bld->append(" SETGUARDS(ret);\n");
...
static void gen_subsat(EBPF::CodeBuilder *bld, const WidthRec *wr) {
    ...
    if (bits <= 64) {
        unsigned long long int max;
        max = (bits < 64) ? (1ULL << bits) - 1ULL : 0xffffffffffffffffULL;
        if (wr->sarith.issigned) {  // let the optimizer delete the &0x...ULL when appropriate
            bld->appendFormat(" ret = (lhs - rhs) & 0x%llxULL;\n", max);
            bld->appendFormat(
...
static void gen_cmp(EBPF::CodeBuilder *bld, const WidthRec *wr) {
    ...
    bld->appendFormat(
        ...
        (wr->cmp.cmp & CMP_SIGNED) ? "s" : "u", w, cmp_string(wr->cmp.cmp & CMP_BASE), w, w);
    bld->append("{\n" /*}*/);
    switch (wr->cmp.cmp) {
        case CMP_EQ:
            bld->append(" return(");
...
bool Backend::process() {
    ...
    ScanWidths *sw = new ScanWidths(typeMap);
