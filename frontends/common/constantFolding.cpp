...
DoConstantFolding::Result DoConstantFolding::setContains(const IR::Expression* keySet,
    ...
    if (select->is<IR::BoolLiteral>()) {
        ...
        if (key == nullptr) {
            ::error(ErrorType::ERR_TYPE_ERROR, "%1%: expression must evaluate to a constant", keySet);
            return Result::No;
        }
    ...
    if (auto kc = keySet->to<IR::Constant>()) {
        ...
        if (left == nullptr) {
            ::error(ErrorType::ERR_INVALID, "%1%: expression must evaluate to a constant", range->left);
            return Result::DontKnow;
        }
        auto right = getConstant(range->right);
        if (right == nullptr) {
            ::error(ErrorType::ERR_INVALID, "%1%: expression must evaluate to a constant", range->right);
            return Result::DontKnow;
        }
