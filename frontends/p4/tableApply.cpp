...
const IR::P4Table *TableApplySolver::isHit(const IR::Expression *expression, ReferenceMap *refMap,
    ...
    if (mem->member != IR::Type_Table::hit) return nullptr;
    auto mce = mem->expr->to<IR::MethodCallExpression>();
    auto mi = P4::MethodInstance::resolve(mce, refMap, typeMap);
    if (!mi->isApply()) return nullptr;

    auto am = mi->to<P4::ApplyMethod>();
    if (!am->object->is<IR::P4Table>()) return nullptr;
...
const IR::P4Table *TableApplySolver::isMiss(const IR::Expression *expression, ReferenceMap *refMap,
    ...
    auto mem = expression->to<IR::Member>();
    if (!mem->expr->is<IR::MethodCallExpression>()) return nullptr;
    if (mem->member != IR::Type_Table::miss) return nullptr;
    auto mce = mem->expr->to<IR::MethodCallExpression>();
    auto mi = P4::MethodInstance::resolve(mce, refMap, typeMap);
    if (!mi->isApply()) return nullptr;
...
const IR::P4Table *TableApplySolver::isActionRun(const IR::Expression *expression,
    ...
    auto mce = mem->expr->to<IR::MethodCallExpression>();
    if (mce == nullptr) return nullptr;
    auto instance = P4::MethodInstance::resolve(mce, refMap, typeMap);
    if (!instance->is<P4::ApplyMethod>()) return nullptr;
