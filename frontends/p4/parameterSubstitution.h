...
class ParameterSubstitution : public IHasDbPrint {
 ...
 public:
    ...
    Util::Enumerator<IR::Ptr<IR::Parameter>> *getParametersInOrder() const {
        if (paramList == nullptr) return new Util::EmptyEnumerator<IR::Ptr<IR::Parameter>>;
 ...
 private:
    ...
    absl::flat_hash_map<cstring, IR::Ptr<IR::Argument>> parameterValues;
    /// Map from parameter name to parameter.
    absl::flat_hash_map<cstring, IR::Ptr<IR::Parameter>> parametersByName;
    ...
    IR::Ptr<IR::ParameterList> paramList = nullptr;
};
