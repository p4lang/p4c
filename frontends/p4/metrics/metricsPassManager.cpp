...
void MetricsPassManager::addMetricPasses(PassManager &pm){
    if (selectedMetrics.count("loc")) pm.addPasses({new LinesOfCodeMetricPass(metrics, isolatedFileName)});
    if (selectedMetrics.count("cyclomatic")) pm.addPasses({new CyclomaticComplexityPass(metrics)});
    if (selectedMetrics.count("halstead")) pm.addPasses({new HalsteadMetricsPass(metrics)});
    if (selectedMetrics.count("nesting-depth")) pm.addPasses({new NestingDepthMetricPass(metrics)});
    if (selectedMetrics.count("header-general")) pm.addPasses({new HeaderMetricsPass(typeMap, metrics)});
    if (selectedMetrics.count("match-action")) pm.addPasses({new MatchActionTableMetricsPass(typeMap, metrics)});
    if (selectedMetrics.count("parser")) pm.addPasses({new ParserMetricsPass(metrics)});
    if (selectedMetrics.count("extern")) pm.addPasses({new ExternalObjectsMetricPass(metrics)});
