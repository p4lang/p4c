...
const IR::Type* TypeInference::canonicalize(const IR::Type* type) {
    ...
    if (type->is<IR::Type_SpecializedCanonical>() ||
        ...
        if (pl == nullptr || tps == nullptr)
            return nullptr;
        if (!checkParameters(pl, true, true))
            return nullptr;
...
const IR::Node* TypeInference::postorder(IR::P4Action* action) {
    ...
    if (pl == nullptr)
        return action;
    if (!checkParameters(action->parameters, true, true))
        return action;
...
TypeInference::containerInstantiation(
    ...
    checkParameters(params, false, !isPackage);  // only packages can have packages as parameters
