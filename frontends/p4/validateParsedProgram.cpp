...
void ValidateParsedProgram::postorder(const IR::Method *m) {
    if (m->name.isDontCare())
        ::p4c::error(ErrorType::ERR_INVALID, "%1%: invalid method/function name.", m->name);
    if (auto ext = findContext<IR::Type_Extern>()) {
        if (m->name == ext->name && m->type->returnType != nullptr)
            ::p4c::error(ErrorType::ERR_INVALID, "%1%: invalid constructor; cannot have a return type.",
                    m);
        if (m->type->returnType == nullptr) {
            if (m->name != ext->name) {
                ::p4c::error(ErrorType::ERR_INVALID, "%1%: invalid type; method has no return type.", m);
                return;
            }
...
void ValidateParsedProgram::postorder(const IR::Annotations *annotations) {
    ...
    for (auto a : annotations->annotations) {
        ...
        if (namesUsed.count(a->name) > 1)
            ::p4c::error(ErrorType::ERR_DUPLICATE, "%1%: duplicate name for structured annotation", a);
    }
}

/// Struct field names cannot be underscore
void ValidateParsedProgram::postorder(const IR::StructField *f) {
    if (f->name.isDontCare()) ::p4c::error(ErrorType::ERR_INVALID, "%1%: invalid field name", f->name);
}
...
void ValidateParsedProgram::postorder(const IR::P4Table *t) {
    ...
    if (ac == nullptr)
        ::p4c::error(ErrorType::ERR_EXPECTED, "%1%: expected '%2%' property", t->name,
...
void ValidateParsedProgram::distinctParameters(const IR::TypeParameters *typeParams,
    ...
    for (auto p : apply->parameters) {
        ...
        if (!inserted) ::p4c::error(ErrorType::ERR_DUPLICATE, "%1% duplicates %2%.", it->second, p);
    }
    for (auto p : constr->parameters) {
        auto it = found.find(p->getName());
        if (it != found.end())
            ::p4c::error(ErrorType::ERR_DUPLICATE, "%1% duplicates %2%.", it->second, p);
    }
}
...
void ValidateParsedProgram::postorder(const IR::Declaration_Variable *decl) {
    if (decl->name.isDontCare())
        ::p4c::error(ErrorType::ERR_INVALID, "%1%: invalid variable name.", decl);
}
...
void ValidateParsedProgram::postorder(const IR::Declaration_Instance *decl) {
    if (!decl->type->is<IR::Type_Name>() && !decl->type->is<IR::Type_Specialized>() &&
        ...
        ::p4c::error(ErrorType::ERR_INVALID, "%1%: invalid type for declaration", decl->type);
    if (decl->name.isDontCare())
        ::p4c::error(ErrorType::ERR_INVALID, "%1%: invalid instance name.", decl);
    ...
    auto inAction = findContext<IR::P4Action>();
    if (inAction != nullptr)
        ::p4c::error(ErrorType::ERR_INVALID, "%1%: declaration. Instantiations not allowed in actions.",
                decl);
}
...
void ValidateParsedProgram::postorder(const IR::Declaration_Constant *decl) {
    if (decl->name.isDontCare())
        ::p4c::error(ErrorType::ERR_INVALID, "%1%: invalid constant name.", decl);
}
    ...
    for (auto c : statement->cases) {
        if (defaultFound != nullptr) {
            if (c->label->is<IR::DefaultExpression>())
                ::p4c::error(ErrorType::ERR_INVALID, "%1% has multiple 'default' labels: %2% and %3%.",
    ...
    for (auto decl : *program->getDeclarations()) {
        ...
        if (existing != nullptr) {
            if (!existing->is<IR::IFunctional>() || !decl->is<IR::IFunctional>() ||
                ...
                ::p4c::error(ErrorType::ERR_DUPLICATE, "%1% duplicates %2%.", decl->getName(),
                        existing->getName());
            }
        } else {
...
void ValidateParsedProgram::postorder(const IR::Dots *dots) {
    ...
    if (context->node->is<IR::ListExpression>()) {
        ...
        if (context->child_index != (int)list->size() - 1)
            ::p4c::error(ErrorType::ERR_INVALID, "%1% must be the last element", dots);
    }
}
