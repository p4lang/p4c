...
table tbl {
	key {
		...
		m.Ingress_key_1 exact
		m.Ingress_key_2 exact
	}
	actions {
		NoAction
		execute_1
...
apply {
	...
	INGRESSPARSERIMPL_ACCEPT :	mov m.Ingress_key m.local_metadata_data1
	mov m.Ingress_key_0 1
	jmpv LABEL_END h.ethernet
	mov m.Ingress_key_0 0
	LABEL_END :	mov m.Ingress_key_1 1
	jmpv LABEL_END_0 h.tcp
	mov m.Ingress_key_1 0
	LABEL_END_0 :	mov m.Ingress_key_2 1
	jmpv LABEL_END_1 h.ipv4
	mov m.Ingress_key_2 0
