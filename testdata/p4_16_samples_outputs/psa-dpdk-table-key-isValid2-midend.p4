...
control ingress(inout headers hdr, inout metadata user_meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    @hidden action psadpdktablekeyisValid2l102() {
        key_0 = hdr.ethernet.isValid() || hdr.ipv4.isValid();
    }
    @hidden table tbl_psadpdktablekeyisValid2l102 {
        actions = {
            psadpdktablekeyisValid2l102();
        }
        const default_action = psadpdktablekeyisValid2l102();
    }
    apply {
        tbl_psadpdktablekeyisValid2l102.apply();
        tbl_0.apply();
    }
}
...
control IngressDeparserImpl(packet_out packet, out empty_metadata_t clone_i2e_meta, out empty_metadata_t resubmit_meta, out empty_metadata_t normal_meta, inout headers hdr, in metadata meta, in psa_ingress_output_metadata_t istd) {
    @hidden action psadpdktablekeyisValid2l143() {
        packet.emit<ethernet_t>(hdr.ethernet);
        packet.emit<ipv4_t>(hdr.ipv4);
        packet.emit<tcp_t>(hdr.tcp);
    }
    @hidden table tbl_psadpdktablekeyisValid2l143 {
        actions = {
            psadpdktablekeyisValid2l143();
        }
        const default_action = psadpdktablekeyisValid2l143();
    }
    apply {
        tbl_psadpdktablekeyisValid2l143.apply();
    }
}
...
control EgressDeparserImpl(packet_out packet, out empty_metadata_t clone_e2e_meta, out empty_metadata_t recirculate_meta, inout headers hdr, in metadata meta, in psa_egress_output_metadata_t istd, in psa_egress_deparser_input_metadata_t edstd) {
    @hidden action psadpdktablekeyisValid2l158() {
        packet.emit<ethernet_t>(hdr.ethernet);
        packet.emit<ipv4_t>(hdr.ipv4);
        packet.emit<tcp_t>(hdr.tcp);
    }
    @hidden table tbl_psadpdktablekeyisValid2l158 {
        actions = {
            psadpdktablekeyisValid2l158();
        }
        const default_action = psadpdktablekeyisValid2l158();
    }
    apply {
        tbl_psadpdktablekeyisValid2l158.apply();
    }
}
