...
parser TopParser(packet_in b, out Parsed_packet p) {
    @name("trill") ValueSet<bit<16>>() trill_0;
    @name("tpid") ValueSet<bit<16>>() tpid_0;
    state start {
        b.extract<Ethernet_h>(p.ethernet);
        transition select(true, p.ethernet.etherType) {
            (true, 16w0x800): parse_ipv4;
            (true, 16w0x806): parse_arp;
            (true, 16w0x86dd): parse_ipv6;
            (true, trill_0.members()): parse_trill;
            (true, tpid_0.members()): parse_vlan_tag;
        }
    }
    state parse_ipv4 {
        transition accept;
