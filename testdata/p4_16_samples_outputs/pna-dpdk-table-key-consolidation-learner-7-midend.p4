...
control MainDeparserImpl(packet_out pkt, in headers_t hdr, in main_metadata_t user_meta, in pna_main_output_metadata_t ostd) {
    @hidden action pnadpdktablekeyconsolidationlearner7l164() {
        pkt.emit<ethernet_t>(hdr.ethernet);
        pkt.emit<ipv4_t>(hdr.ipv4);
    }
    @hidden table tbl_pnadpdktablekeyconsolidationlearner7l164 {
        actions = {
            pnadpdktablekeyconsolidationlearner7l164();
        }
        const default_action = pnadpdktablekeyconsolidationlearner7l164();
    }
    apply {
        tbl_pnadpdktablekeyconsolidationlearner7l164.apply();
    }
}
