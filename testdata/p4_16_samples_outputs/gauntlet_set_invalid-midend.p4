...
control ingress(inout Parsed_packet hdr, inout Metadata meta, inout standard_metadata_t stdmeta) {
    @hidden action gauntlet_set_invalid58() {
        hdr.h.setInvalid();
        hdr.h.setValid();
        hdr.h.b = 8w2;
    }
    @hidden table tbl_gauntlet_set_invalid58 {
        actions = {
            gauntlet_set_invalid58();
        }
        const default_action = gauntlet_set_invalid58();
    }
    apply {
        tbl_gauntlet_set_invalid58.apply();
    }
}
