...
typedef bit<16> EthernetAddress;
struct user_meta_t {
    bit<16> data;
}
...
control MyIC(inout headers_t hdr, inout user_meta_t b, in psa_ingress_input_metadata_t c, inout psa_ingress_output_metadata_t d) {
    ...
    const bit<16> base = 16w2;
    const bit<16> max_val = 16w32;
    action a1() {
        b.data = h.get_hash<bit<16>, tuple<bit<16>, bit<16>>>(16w2, { hdr.ethernet.srcAddr, hdr.ethernet.etherType }, 16w32);
    }
    table tbl {
        key = {
