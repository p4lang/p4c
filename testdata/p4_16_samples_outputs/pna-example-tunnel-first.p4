...
control main_control(inout headers_t hdr, inout local_metadata_t local_metadata, in pna_main_input_metadata_t istd, inout pna_main_output_metadata_t ostd) {
    @name("tunnel_decap") tunnel_decap() tunnel_decap_inst;
    @name("tunnel_encap") tunnel_encap() tunnel_encap_inst;
    apply {
        if (RxPkt(istd)) {
            tunnel_decap_inst.apply(hdr, local_metadata);
        } else {
            tunnel_encap_inst.apply(hdr, local_metadata, istd);
        }
    }
}
