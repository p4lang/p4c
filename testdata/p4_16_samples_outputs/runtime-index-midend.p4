...
control ingress(inout headers hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    @hidden action runtimeindex68() {
        hdr.vector[0].e = hdr.pool[1].val;
        hdr.pool[hdr.ml.idx].val = hdr.pool[1].val;
        hdr.pool[hdr.ml.idx].val = hdr.pool[hdr.ml.idx].val + 32w1;
        standard_metadata.egress_spec = standard_metadata.ingress_port;
    }
    @hidden table tbl_runtimeindex68 {
        actions = {
            runtimeindex68();
        }
        const default_action = runtimeindex68();
    }
    apply {
        tbl_runtimeindex68.apply();
    }
}
