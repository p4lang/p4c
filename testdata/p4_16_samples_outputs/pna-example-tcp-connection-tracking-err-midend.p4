...
control MainControlImpl(inout headers_t hdr, inout metadata_t meta, in pna_main_input_metadata_t istd, inout pna_main_output_metadata_t ostd) {
    ...
    @hidden action pnaexampletcpconnectiontrackingerr254() {
    ...
    @hidden table tbl_pnaexampletcpconnectiontrackingerr254 {
        actions = {
            pnaexampletcpconnectiontrackingerr254();
        }
        const default_action = pnaexampletcpconnectiontrackingerr254();
    }
    apply {
        ...
        if (hdr.ipv4.isValid() && hdr.tcp.isValid()) {
            tbl_pnaexampletcpconnectiontrackingerr254.apply();
...
control MainDeparserImpl(packet_out pkt, in headers_t hdr, in metadata_t meta, in pna_main_output_metadata_t ostd) {
    @hidden action pnaexampletcpconnectiontrackingerr329() {
        pkt.emit<ethernet_t>(hdr.eth);
    }
    @hidden table tbl_pnaexampletcpconnectiontrackingerr329 {
        actions = {
            pnaexampletcpconnectiontrackingerr329();
        }
        const default_action = pnaexampletcpconnectiontrackingerr329();
    }
    apply {
        tbl_pnaexampletcpconnectiontrackingerr329.apply();
    }
}
