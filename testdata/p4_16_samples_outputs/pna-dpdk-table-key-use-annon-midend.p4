...
control MainControlImpl(inout headers_t hdr, inout main_metadata_t user_meta, in pna_main_input_metadata_t istd, inout pna_main_output_metadata_t ostd) {
    ...
    @name("MainControlImpl.ipv4_da2") table ipv4_da2_0 {
        key = {
            hdr.ipv4.dstAddr: exact @name("ipv4_addr_1") ;
        }
        actions = {
            @tableonly next_hop2();
...
control MainDeparserImpl(packet_out pkt, in headers_t hdr, in main_metadata_t user_meta, in pna_main_output_metadata_t ostd) {
    @hidden action pnadpdktablekeyuseannon185() {
        pkt.emit<ethernet_t>(hdr.ethernet);
        pkt.emit<ipv4_t>(hdr.ipv4);
    }
    @hidden table tbl_pnadpdktablekeyuseannon185 {
        actions = {
            pnadpdktablekeyuseannon185();
        }
        const default_action = pnadpdktablekeyuseannon185();
    }
    apply {
        tbl_pnadpdktablekeyuseannon185.apply();
    }
}
