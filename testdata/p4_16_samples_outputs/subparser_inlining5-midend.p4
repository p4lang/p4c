...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @hidden action subparser_inlining5l94() {
        standard_metadata.egress_port = 9w2;
    }
    @hidden action subparser_inlining5l96() {
        standard_metadata.egress_port = 9w3;
    }
    @hidden action subparser_inlining5l98() {
        standard_metadata.egress_port = 9w10;
    }
    @hidden table tbl_subparser_inlining5l94 {
        actions = {
            subparser_inlining5l94();
        }
        const default_action = subparser_inlining5l94();
    }
    @hidden table tbl_subparser_inlining5l96 {
        actions = {
            subparser_inlining5l96();
        }
        const default_action = subparser_inlining5l96();
    }
    @hidden table tbl_subparser_inlining5l98 {
        actions = {
            subparser_inlining5l98();
        }
        const default_action = subparser_inlining5l98();
    }
    apply {
        if (hdr.h2.isValid()) {
            tbl_subparser_inlining5l94.apply();
        } else if (hdr.h3.isValid()) {
            tbl_subparser_inlining5l96.apply();
        } else {
            tbl_subparser_inlining5l98.apply();
        }
    }
}
