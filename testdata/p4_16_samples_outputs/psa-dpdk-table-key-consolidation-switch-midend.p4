...
control MyID(packet_out buffer, out EMPTY a, out EMPTY b, out EMPTY c, inout headers_t hdr, in user_meta_t e, in psa_ingress_output_metadata_t f) {
    @hidden action psadpdktablekeyconsolidationswitch146() {
        buffer.emit<ethernet_t>(hdr.ethernet);
    }
    @hidden table tbl_psadpdktablekeyconsolidationswitch146 {
        actions = {
            psadpdktablekeyconsolidationswitch146();
        }
        const default_action = psadpdktablekeyconsolidationswitch146();
    }
    apply {
        tbl_psadpdktablekeyconsolidationswitch146.apply();
    }
}
