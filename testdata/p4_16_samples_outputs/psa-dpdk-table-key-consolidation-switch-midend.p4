...
control MyID(packet_out buffer, out EMPTY a, out EMPTY b, out EMPTY c, inout headers_t hdr, in user_meta_t e, in psa_ingress_output_metadata_t f) {
    @hidden action psadpdktablekeyconsolidationswitch152() {
        buffer.emit<ethernet_t>(hdr.ethernet);
    }
    @hidden table tbl_psadpdktablekeyconsolidationswitch152 {
        actions = {
            psadpdktablekeyconsolidationswitch152();
        }
        const default_action = psadpdktablekeyconsolidationswitch152();
    }
    apply {
        tbl_psadpdktablekeyconsolidationswitch152.apply();
    }
}
