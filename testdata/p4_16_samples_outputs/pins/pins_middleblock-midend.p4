...
control ingress(inout headers_t headers, inout local_metadata_t local_metadata, inout standard_metadata_t standard_metadata) {
    ...
    @id(0x01000109) @sai_action(SAI_PACKET_ACTION_DROP) @name(".acl_drop") action acl_drop_0() {
  ...
  ") @name("ingress.acl_ingress.acl_ingress_table") table acl_ingress_acl_ingress_table {
    ...
    @hidden action pins_middleblock679() {
    ...
    @hidden action pins_middleblock684() {
    ...
    @hidden action pins_middleblock586() {
        acl_ingress_ttl = 8w0;
        acl_ingress_dscp = 6w0;
        acl_ingress_ecn = 2w0;
        acl_ingress_ip_protocol = 8w0;
    }
    @hidden action pins_middleblock645() {
        key_2 = headers.ipv4.isValid() || headers.ipv6.isValid();
    }
    @hidden action pins_middleblock557() {
        mark_to_drop(standard_metadata);
    }
    @hidden action pins_middleblock559() {
        headers.ipv4.ttl = headers.ipv4.ttl + 8w255;
    }
    @hidden action pins_middleblock564() {
        mark_to_drop(standard_metadata);
    }
    @hidden action pins_middleblock566() {
        headers.ipv6.hop_limit = headers.ipv6.hop_limit + 8w255;
    }
    @hidden action pins_middleblock524() {
        clone_preserving_field_list(CloneType.I2E, mirroring_clone_pre_session, 8w1);
    }
    @hidden table tbl_pins_middleblock694 {
        actions = {
            pins_middleblock694();
        }
        const default_action = pins_middleblock694();
    }
    @hidden table tbl_pins_middleblock736 {
        actions = {
            pins_middleblock736();
        }
        const default_action = pins_middleblock736();
    }
    @hidden table tbl_pins_middleblock738 {
        actions = {
            pins_middleblock738();
        }
        const default_action = pins_middleblock738();
    }
    @hidden table tbl_pins_middleblock717 {
        actions = {
            pins_middleblock717();
        }
        const default_action = pins_middleblock717();
    }
    @hidden table tbl_pins_middleblock747 {
        actions = {
            pins_middleblock747();
        }
        const default_action = pins_middleblock747();
    }
    @hidden table tbl_pins_middleblock262 {
        actions = {
            pins_middleblock262();
        }
        const default_action = pins_middleblock262();
    }
    @hidden table tbl_pins_middleblock586 {
        actions = {
            pins_middleblock586();
        }
        const default_action = pins_middleblock586();
    }
    @hidden table tbl_pins_middleblock679 {
        actions = {
            pins_middleblock679();
        }
        const default_action = pins_middleblock679();
    }
    @hidden table tbl_pins_middleblock684 {
        actions = {
            pins_middleblock684();
        }
        const default_action = pins_middleblock684();
    }
    @hidden table tbl_pins_middleblock645 {
        actions = {
            pins_middleblock645();
        }
        const default_action = pins_middleblock645();
    }
    @hidden table tbl_pins_middleblock557 {
        actions = {
            pins_middleblock557();
        }
        const default_action = pins_middleblock557();
    }
    @hidden table tbl_pins_middleblock559 {
        actions = {
            pins_middleblock559();
        }
        const default_action = pins_middleblock559();
    }
    @hidden table tbl_pins_middleblock564 {
        actions = {
            pins_middleblock564();
        }
        const default_action = pins_middleblock564();
    }
    @hidden table tbl_pins_middleblock566 {
        actions = {
            pins_middleblock566();
        }
        const default_action = pins_middleblock566();
    }
    @hidden table tbl_pins_middleblock524 {
        actions = {
            pins_middleblock524();
        }
        const default_action = pins_middleblock524();
    }
    apply {
        tbl_pins_middleblock694.apply();
        if (headers.ipv4.isValid()) {
            tbl_pins_middleblock736.apply();
        } else if (headers.ipv6.isValid()) {
            tbl_pins_middleblock738.apply();
        }
        tbl_pins_middleblock717.apply();
        acl_pre_ingress_acl_pre_ingress_table.apply();
        tbl_pins_middleblock747.apply();
        l3_admit_l3_admit_table.apply();
        tbl_pins_middleblock262.apply();
        ...
        tbl_pins_middleblock586.apply();
        if (headers.ipv4.isValid()) {
            tbl_pins_middleblock679.apply();
        } else if (headers.ipv6.isValid()) {
            tbl_pins_middleblock684.apply();
        }
        tbl_pins_middleblock645.apply();
        acl_ingress_acl_ingress_table.apply();
        if (local_metadata._admit_to_l30) {
            if (headers.ipv4.isValid()) {
                if (headers.ipv4.ttl <= 8w1) {
                    tbl_pins_middleblock557.apply();
                } else {
                    tbl_pins_middleblock559.apply();
                }
            }
            if (headers.ipv6.isValid()) {
                if (headers.ipv6.hop_limit <= 8w1) {
                    tbl_pins_middleblock564.apply();
                } else {
                    tbl_pins_middleblock566.apply();
                }
            }
        }
...
control egress(inout headers_t headers, inout local_metadata_t local_metadata, inout standard_metadata_t standard_metadata) {
    @hidden action pins_middleblock576() {
    ...
    @hidden action pins_middleblock449() {
    ...
    @hidden table tbl_pins_middleblock576 {
        actions = {
            pins_middleblock576();
        }
        const default_action = pins_middleblock576();
    }
    @hidden table tbl_pins_middleblock449 {
        actions = {
            pins_middleblock449();
        }
        const default_action = pins_middleblock449();
    }
    apply {
        if (local_metadata._admit_to_l30) {
            tbl_pins_middleblock576.apply();
        }
        if (standard_metadata.instance_type == 32w1) {
            tbl_pins_middleblock449.apply();
        }
    }
}
