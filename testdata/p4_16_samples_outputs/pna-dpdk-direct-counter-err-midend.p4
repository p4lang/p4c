...
control MainDeparserImpl(packet_out pkt, in headers_t hdr, in main_metadata_t user_meta, in pna_main_output_metadata_t ostd) {
    @hidden action pnadpdkdirectcountererr125() {
        pkt.emit<ethernet_t>(hdr.ethernet);
        pkt.emit<ipv4_t>(hdr.ipv4);
    }
    @hidden table tbl_pnadpdkdirectcountererr125 {
        actions = {
            pnadpdkdirectcountererr125();
        }
        const default_action = pnadpdkdirectcountererr125();
    }
    apply {
        tbl_pnadpdkdirectcountererr125.apply();
    }
}
