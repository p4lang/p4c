...
control MyIngressControl(inout headers_t hdrs, inout user_meta_data_t meta, in psa_ingress_input_metadata_t c, inout psa_ingress_output_metadata_t d) {
    ...
    @hidden action psasubtractinst1l58() {
        var_0 = 3w2;
        d.egress_port = (bit<32>)c.ingress_port ^ 32w1;
    }
    @hidden table tbl_psasubtractinst1l58 {
        actions = {
            psasubtractinst1l58();
        }
        const default_action = psasubtractinst1l58();
    }
    apply {
        tbl_psasubtractinst1l58.apply();
        stub_0.apply();
    }
}
...
control MyIngressDeparser(packet_out pkt, out EMPTY a, out EMPTY b, out EMPTY c, inout headers_t hdr, in user_meta_data_t e, in psa_ingress_output_metadata_t f) {
    @hidden action psasubtractinst1l92() {
        pkt.emit<ethernet_t>(hdr.ethernet);
    }
    @hidden table tbl_psasubtractinst1l92 {
        actions = {
            psasubtractinst1l92();
        }
        const default_action = psasubtractinst1l92();
    }
    apply {
        tbl_psasubtractinst1l92.apply();
    }
}
