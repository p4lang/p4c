...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    state Subparser_sp1 {
        ...
        inout_hdr_0_h1.f = p_shdr_h1.f;
        inout_hdr_0_h1.setValid();
        transition p0_0;
    }
    state Subparser_sp2 {
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @hidden action subparser_inlining13l100() {
        standard_metadata.egress_port = 9w2;
    }
    @hidden action subparser_inlining13l102() {
        standard_metadata.egress_port = 9w3;
    }
    @hidden action subparser_inlining13l104() {
        standard_metadata.egress_port = 9w10;
    }
    @hidden table tbl_subparser_inlining13l100 {
        actions = {
            subparser_inlining13l100();
        }
        const default_action = subparser_inlining13l100();
    }
    @hidden table tbl_subparser_inlining13l102 {
        actions = {
            subparser_inlining13l102();
        }
        const default_action = subparser_inlining13l102();
    }
    @hidden table tbl_subparser_inlining13l104 {
        actions = {
            subparser_inlining13l104();
        }
        const default_action = subparser_inlining13l104();
    }
    apply {
        if (hdr.h2.isValid()) {
            tbl_subparser_inlining13l100.apply();
        } else if (hdr.h3.isValid()) {
            tbl_subparser_inlining13l102.apply();
        } else {
            tbl_subparser_inlining13l104.apply();
        }
    }
}
