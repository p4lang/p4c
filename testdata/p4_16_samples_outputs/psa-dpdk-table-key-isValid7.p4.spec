...
struct metadata {
	...
	bit<8> Ingress_tbl_eth_11
	bit<8> Ingress_tbl_tcp_12
	bit<8> Ingress_tbl_ipv_13
}
...
table tbl {
	key {
		m.Ingress_tbl_eth_11 exact
		m.Ingress_tbl_tcp_12 exact
		m.Ingress_tbl_ipv_13 exact
	}
	actions {
		NoAction
		execute_1
...
apply {
	...
	INGRESSPARSERIMPL_ACCEPT :	mov m.Ingress_tbl_eth_11 1
	jmpv LABEL_END h.ethernet
	mov m.Ingress_tbl_eth_11 0
	LABEL_END :	mov m.Ingress_tbl_tcp_12 1
	jmpv LABEL_END_0 h.tcp
	mov m.Ingress_tbl_tcp_12 0
	LABEL_END_0 :	mov m.Ingress_tbl_ipv_13 1
	jmpv LABEL_END_1 h.ipv4
	mov m.Ingress_tbl_ipv_13 0
