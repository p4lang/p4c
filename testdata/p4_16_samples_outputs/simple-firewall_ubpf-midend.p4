...
control pipe(inout Headers_t headers, inout metadata meta, inout standard_metadata std_meta) {
    ...
    @hidden action simplefirewall_ubpf116() {
        hash<tuple_0>(meta._conn_id2, HashAlgorithm.lookup3, (tuple_0){f0 = headers.ipv4.srcAddr,f1 = headers.ipv4.dstAddr});
    }
    @hidden action simplefirewall_ubpf118() {
        hash<tuple_0>(meta._conn_id2, HashAlgorithm.lookup3, (tuple_0){f0 = headers.ipv4.dstAddr,f1 = headers.ipv4.srcAddr});
    }
    @hidden action simplefirewall_ubpf121() {
        meta._connInfo_s0 = conn_state_0.read(meta._conn_id2);
        meta._connInfo_srv_addr1 = conn_srv_addr_0.read(meta._conn_id2);
    }
    @hidden table tbl_simplefirewall_ubpf116 {
        actions = {
            simplefirewall_ubpf116();
        }
        const default_action = simplefirewall_ubpf116();
    }
    @hidden table tbl_simplefirewall_ubpf118 {
        actions = {
            simplefirewall_ubpf118();
        }
        const default_action = simplefirewall_ubpf118();
    }
    @hidden table tbl_simplefirewall_ubpf121 {
        actions = {
            simplefirewall_ubpf121();
        }
        const default_action = simplefirewall_ubpf121();
    }
    ...
    apply {
        if (headers.tcp.isValid()) {
            if (headers.ipv4.srcAddr < headers.ipv4.dstAddr) {
                tbl_simplefirewall_ubpf116.apply();
            } else {
                tbl_simplefirewall_ubpf118.apply();
            }
            tbl_simplefirewall_ubpf121.apply();
...
control dprs(packet_out packet, in Headers_t headers) {
    @hidden action simplefirewall_ubpf163() {
        packet.emit<Ethernet_t>(headers.ethernet);
        packet.emit<Ipv4_t>(headers.ipv4);
        packet.emit<Tcp_t>(headers.tcp);
    }
    @hidden table tbl_simplefirewall_ubpf163 {
        actions = {
            simplefirewall_ubpf163();
        }
        const default_action = simplefirewall_ubpf163();
    }
    apply {
        tbl_simplefirewall_ubpf163.apply();
    }
}
