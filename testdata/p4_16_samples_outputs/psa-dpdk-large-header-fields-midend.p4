...
control MyIngressControl(inout headers_t hdr, inout user_meta_data_t m, in psa_ingress_input_metadata_t c, inout psa_ingress_output_metadata_t d) {
    ...
    @hidden action psadpdklargeheaderfields36() {
        flg = 8w0;
    }
    @hidden table tbl_psadpdklargeheaderfields36 {
        actions = {
            psadpdklargeheaderfields36();
        }
        const default_action = psadpdklargeheaderfields36();
    }
    apply {
        tbl_psadpdklargeheaderfields36.apply();
        stub.apply();
    }
}
...
control MyIngressDeparser(packet_out pkt, out EMPTY a, out EMPTY b, out EMPTY c, inout headers_t hdr, in user_meta_data_t e, in psa_ingress_output_metadata_t f) {
    @hidden action psadpdklargeheaderfields66() {
        pkt.emit<ethernet_t>(hdr.ethernet);
    }
    @hidden table tbl_psadpdklargeheaderfields66 {
        actions = {
            psadpdklargeheaderfields66();
        }
        const default_action = psadpdklargeheaderfields66();
    }
    apply {
        tbl_psadpdklargeheaderfields66.apply();
    }
}
