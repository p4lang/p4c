                ...
                if (value != NULL) {
                    ...
                    switch (value->action) {
                        case INGRESS_NH_TABLE_ACT_INGRESS_METER_EXEC: 
                            {
/* global_meter_0.execute() */
                                __builtin_memset(&ext_params, 0, sizeof(struct p4tc_ext_bpf_params));
                                ext_params.pipe_id = p4tc_filter_fields.pipeid;
                                ext_params.ext_id = 0x1C000000;
                                ext_params.flags = P4TC_EXT_METER_DIRECT;
                                ext_params.tbl_id = 1;

                                bpf_p4tc_extern_meter_pkts(skb, &ext_params, sizeof(ext_params), &key, sizeof(key));
;
                            }
                            break;
