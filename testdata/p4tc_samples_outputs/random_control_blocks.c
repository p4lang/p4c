...
#define MAIN_NH_TABLE_ACT_MAIN_SEND_NH 1
#define MAIN_NH_TABLE_ACT_MAIN_DROP 2
    ...
    meta = &(hdrMd->cpumap_usermeta);
{
        u8 hit;
        {
            hdr->rand.f1 = /* r1_0.read() */
(bpf_get_prandom_u32() & 0x7fu);
                        hdr->rand.f2 = /* r2_0.read() */
(2147483649u + (bpf_get_prandom_u32() % 5u));
            /* nh_table_0.apply() */
            {
                /* construct key */
                ...
                if (value != NULL) {
                    /* run action */
                    switch (value->action) {
                        case MAIN_NH_TABLE_ACT_MAIN_SEND_NH: 
                            {
storePrimitive64((u8 *)&hdr->eth.srcAddr[0], 48, getPrimitive64((u8 *)(value->u.Main_send_nh.smac), 48));
                                storePrimitive64((u8 *)&hdr->eth.dstAddr[0], 48, getPrimitive64((u8 *)(value->u.Main_send_nh.dmac), 48));
                                                                hdr->rand.f3 = /* r3_0.read() */
(256u + (bpf_get_prandom_u32() & 0x3u));
                                /* send_to_port(value->u.Main_send_nh.port_id) */
...
;
        ...
        if (hdr->rand.ebpf_valid) outHeaderLength += 64;
...
;// emitting header random_t
        if (hdr->rand.ebpf_valid) {
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits + 64)) {
                return TC_ACT_SHOT;
            }
            
            hdr->rand.f1 = bpf_htons(hdr->rand.f1);
            ebpf_byte = ((char*)(&hdr->rand.f1))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char*)(&hdr->rand.f1))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            hdr->rand.f2 = htonl(hdr->rand.f2);
            ebpf_byte = ((char*)(&hdr->rand.f2))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_byte = ((char*)(&hdr->rand.f2))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char*)(&hdr->rand.f2))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char*)(&hdr->rand.f2))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 32;

            hdr->rand.f3 = bpf_htons(hdr->rand.f3);
            ebpf_byte = ((char*)(&hdr->rand.f3))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char*)(&hdr->rand.f3))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;
