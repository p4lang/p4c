...
/* extract(hdr->udp) */
            ...
            __builtin_memcpy(&hdr->udp.src_port, pkt + BYTES(ebpf_packetOffsetInBits), 2);
            ebpf_packetOffsetInBits += 16;

            __builtin_memcpy(&hdr->udp.dst_port, pkt + BYTES(ebpf_packetOffsetInBits), 2);
            ebpf_packetOffsetInBits += 16;

            __builtin_memcpy(&hdr->udp.length, pkt + BYTES(ebpf_packetOffsetInBits), 2);
            ebpf_packetOffsetInBits += 16;

            __builtin_memcpy(&hdr->udp.checksum, pkt + BYTES(ebpf_packetOffsetInBits), 2);
            ebpf_packetOffsetInBits += 16;
