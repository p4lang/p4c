...
;
            tmp_0 = /* ck_0.get() */
((u16) (~ck_0_state));/* verify(hdr->ipv4.hdrChecksum == bpf_htons(tmp_0), BadIPv4HeaderChecksum) */
            if (!(hdr->ipv4.hdrChecksum == bpf_htons(tmp_0))) {
                ebpf_errorCode = BadIPv4HeaderChecksum;
                goto reject;
            }
;
            hdr->ipv4.hdrChecksum = bpf_htons(/* ck_0.get_state() */
