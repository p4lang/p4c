...
newtype bit<PORT_BITWIDTH> PortId_t;
...
// newtype branch of p4c:
//psa-portid-using-newtype1.p4(16): error: cast: Cannot evaluate initializer for constant

//const PortId_t PSA_CPU_PORT = 9w192;  // target-specific
// With no cast on unsigned int literal, I get this error:
//psa-portid-using-newtype1.p4(14): error: PSA_CPU_PORT: Cannot unify bit<9> to PortId_t
//newtype bit<9> PortId_t;
//                        ^
//psa-portid-using-newtype1.p4(14)
//newtype bit<9> PortId_t;
//               ^^^^^^^^
...
@p4runtime_translate("port")
newtype bit<32> PortIdInHeader_t;
...
// Note: all of these casts are necessary with the current newtype
...
// locally-modified version of v1model.p4, called v1model-newtype.p4.
...
header packet_in_header_t {
    ...
    // PortIdInHeader_t is defined as a newtype with a
...
//sa-portid-using-newtype2.p4(207): error: AssignmentStatement: Cannot unify bit<9> to PortId_t
//        standard_metadata.egress_spec = ((PortIdUInt_t) standard_metadata.egress_spec) + 1;
//                                      ^
//psa-portid-using-newtype2.p4(14)
//newtype bit<9> PortId_t;
//               ^^^^^^^^
        //standard_metadata.egress_spec = ((PortIdUInt_t) standard_metadata.egress_spec) + 1;
