                ...
                if (value != NULL) {
                    ...
                    switch (value->action) {
                        case INGRESS_NH_TABLE_ACT_INGRESS_REFLECT: 
                            {
storePrimitive64((u8 *)&tmp_0[0], 48, getPrimitive64((u8 *)(hdr->ethernet.dstAddr), 48));
                                storePrimitive64((u8 *)&hdr->ethernet.dstAddr[0], 48, getPrimitive64((u8 *)(hdr->ethernet.srcAddr), 48));
                                storePrimitive64((u8 *)&hdr->ethernet.srcAddr[0], 48, getPrimitive64((u8 *)(tmp_0), 48));
...
;
        ...
        if (hdr->udp.ebpf_valid) outHeaderLength += 64;
...
;// emitting header udp_t
        if (hdr->udp.ebpf_valid) {
            
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits) + BYTES(16)) return TC_ACT_SHOT;
            ...
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits) + BYTES(16)) return TC_ACT_SHOT;
            hdr->udp.csum = bpf_htons(hdr->udp.csum);
            ebpf_byte = ((char*)(&hdr->udp.csum))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char*)(&hdr->udp.csum))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;
