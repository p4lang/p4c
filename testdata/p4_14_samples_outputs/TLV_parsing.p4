...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name("parse_ipv4") state parse_ipv4 {
        ...
        meta.my_metadata.parse_ipv4_counter = hdr.ipv4_base.ihl * 4w4 - 4w4;
    ...
    @name("parse_ipv4_option_timestamp") state parse_ipv4_option_timestamp {
        packet.extract(hdr.ipv4_option_timestamp);
...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name("format_options_timestamp") action format_options_timestamp() {
        hdr.ipv4_option_NOP.pop_front(3);
        hdr.ipv4_option_EOL.pop_front(3);
        hdr.ipv4_base.ihl = 8w5 + (hdr.ipv4_option_timestamp.len >> 8w3);
    }
    @name("format_options_both") action format_options_both() {
        ...
        hdr.ipv4_base.ihl = 8w8 + (hdr.ipv4_option_timestamp.len >> 8w2);
    }
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        ...
        packet.emit(hdr.ipv4_base);
        packet.emit(hdr.ipv4_option_EOL);
        packet.emit(hdr.ipv4_option_NOP);
        packet.emit(hdr.ipv4_option_security);
        packet.emit(hdr.ipv4_option_timestamp);
    }
}
