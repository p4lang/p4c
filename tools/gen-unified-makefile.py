...
#
# This script examines the source tree, starting at the current directory, and
# automatically generates an automake file which can be included by Makefile.am
# to enable unified compilation. The idea of unified compilation is to create a
# "unified" C++ source file which simply #include's other source files.  Rather
# than compiling the original source files, we compile only the unified file,
# which greatly reduces compilation overhead in terms of I/O, duplicate template
...
# effect.
#
# The output of the script is written to stdout, so all that's necessary to use
# it is to run it from the root of the source tree before automake is run,
# redirect the output to a file, and then include that file from your
# Makefile.am. The script requires that Makefile.am also set a GEN_UNIFIED_CPP
...
def generate_preamble(script_name, max_chunk_size):
    print('###################################################################')
    print('# Unified compilation automake file.')
    print('# Generated by:', script_name)
    print('# Maximum chunk size:', max_chunk_size)
...
def generate_rules(target, unified_file_chunks, nonunified_file_set):
    # Non-unified files are just passed through directly to 'foo_SOURCES'.
    nonunified_files = ' '.join(nonunified_file_set)
    print('{}_SOURCES = {}'.format(target, nonunified_files))
    print()
    ...
    for index in xrange(0, len(unified_file_chunks)):
        ...
        print('{}: $(GEN_UNIFIED_CPP) {}'.format(chunk_target, files_in_chunk))
        print('\t@$(GEN_UNIFIED_CPP) {} > $@'.format(files_in_chunk))
        print()
        print('{}_SOURCES += {}'.format(target, chunk_target))
        print()

def main(argv):
    # The chunk size determines the maximum number of files included by each of
    # the .cpp files we'll generate.
    max_chunk_size = 10
    if len(argv) > 1:
        max_chunk_size = argv[1]
    ...
    generate_preamble(argv[0], max_chunk_size)
    for target in unified_targets:
        unified_file_chunks = chunk(max_chunk_size, unified_file_sets[target])
        generate_rules(target, unified_file_chunks, nonunified_file_sets[target])
