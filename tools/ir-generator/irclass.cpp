...
void IrDefinitions::generate(std::ostream &t, std::ostream &out, std::ostream &impl) const {
    ...
    // Add some specials:
    t << "  NK_IDeclaration = " << nkId++ << ",\n";
    t << "  NK_VectorBase = " << nkId++ << ",\n";
    t << "  NK_VectorT = UINT64_C(1) << 56,\n"
      << "  NK_IndexedVectorT = UINT64_C(2) << 56,\n"
      << "  NK_FirstAuto = UINT64_C(0xFF) << 56\n"
      << "};\n";
...
void IrClass::generate_hdr(std::ostream &out) const {
    ...
    if (kind != NodeKind::Nested) {
        out << indent << "DECLARE_TYPEINFO_WITH_TYPEID(" << name << ", NK_" << name;
        if (!concreteParent)
            out << ", " << (kind != NodeKind::Interface ? "Node" : "INode");
        for (const auto *p : parentClasses)
            out << ", " << p->qualified_name(containedIn);
        out << ");" << std::endl;
    }
