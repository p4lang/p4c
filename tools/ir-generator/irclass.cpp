...
void IrDefinitions::generate(std::ostream &t, std::ostream &out, std::ostream &impl) const {
    ...
    t << "enum class NodeKind : uint64_t {\n"
      << "  Auto = 0,\n"
      << "  INode = 1,\n"
      << "  Node = 2,\n";

    unsigned nkId = 3;
    for (auto cls : *getClasses()) t << "  " << cls->name << " = " << nkId++ << ",\n";

    // Add some specials:
    t << "  IDeclaration = " << nkId++ << ",\n";
    t << "  VectorBase = " << nkId++ << ",\n";
    t << "  VectorT = UINT64_C(1) << 56,\n"
      << "  IndexedVectorT = UINT64_C(2) << 56,\n"
      << "  FirstAuto = UINT64_C(0xFF) << 56\n"
      << "};\n"
      << " inline bool operator==(uint64_t lhs, NodeKind rhs) { return lhs == uint64_t(rhs); }\n"
      << " inline bool operator==(NodeKind lhs, uint64_t rhs) { return uint64_t(lhs) == rhs; }\n"
      << " inline bool operator!=(uint64_t lhs, NodeKind rhs) { return lhs != uint64_t(rhs); }\n"
      << " inline bool operator!=(NodeKind lhs, uint64_t rhs) { return uint64_t(lhs) != rhs; }\n";
    t << "}  // namespace IR" << std::endl;
}
...
void IrClass::generate_hdr(std::ostream &out) const {
    ...
    if (kind != NodeKind::Nested) {
        out << indent << "DECLARE_TYPEINFO_WITH_TYPEID(" << name << ", NodeKind::" << name;
