...
struct COWfieldref<T, const U *, field> {
    ...
    template <typename V>
        requires(std::is_base_of_v<V, U> && !std::same_as<V, U>)
    void set(const V *val) const {
        *this = val ? val->template checkedTo<U>() : (U *)nullptr;
    }
...
struct COW_element_ref {
    ...
    void set(const Node *val)
        requires std::is_pointer_v<U>
    {
        set(val ? val->checkedTo<typename std::remove_pointer<U>::type>() : (U) nullptr);
    }
...
struct COWfieldref<T, safe_vector<U>, field> {
    ...
    void visit_children(Visitor &, const char * = nullptr) { BUG("TBD"); }
