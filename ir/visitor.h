...
class SplitFlowVisit : public SplitFlowVisit_base {
 ...
 public:
    ...
    template <class T, const N *T::*field>
    void addNode(IR::COWfieldref<T, const N *, field> &f) {
        // FIXME -- need to save vector of different COWfieldref types so we can iterate over
        // them.  Create a base class with virtual methods with derived (template) types for
        // each COWfieldref type (derived contains a copied instance of the COWfieldref and
        // virtual overrides, using placement new to build the right type in the vector?)
        // Or maybe use std::any?
        BUG("TBD");
    }
