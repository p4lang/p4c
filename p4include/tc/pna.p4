...
@tc_acl("RUP:RUXP") extern Meter<S> {
  Meter(@tc_numel bit<32> n_meters, PNA_MeterType_t type);
  ...
  @tc_md_exec PNA_MeterColor_t execute(in S index, in PNA_MeterColor_t color);
  ...
  @tc_md_exec PNA_MeterColor_t execute(in S index);
}

struct tc_ControlPath_Meter<S> {
   @tc_key S index;
   @tc_data bit<64> cir;
   @tc_data bit<64> cbs;
   @tc_data bit<64> pir;
...
@tc_acl("RUP:RUXP") extern DirectMeter {
  DirectMeter(PNA_MeterType_t type);
  // See the corresponding methods for extern Meter.
  @tc_md_exec PNA_MeterColor_t execute(in PNA_MeterColor_t color);
  @tc_md_exec PNA_MeterColor_t execute();
}
struct tc_ControlPath_DirectMeter {
   @tc_key bit<32> index;
   @tc_data bit<64> cir;
   @tc_data bit<64> cbs;
   @tc_data bit<64> pir;
