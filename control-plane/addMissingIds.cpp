...
/*const IR::Type_Extern *MissingIdAssigner::postorder(IR::Type_Extern *ext) {
    CHECK_NULL(symbols);
    if (ext->name != "action_profile") {
        return ext;
    }
    auto anno = ControlPlaneAPI::getIdAnnotation(ext);
    if (!anno) {
        const auto *annos = ext->getAnnotations();
        auto *newAnnos = annos->clone();
        IR::Vector<IR::Expression> annoExprs;
        auto symbolId = symbols->getId(
            ControlPlaneAPI::Standard::SymbolType::ACTION_PROFILE(), ext);
        const auto *idConst =
            new IR::Constant(new IR::Type_Bits(ID_BIT_WIDTH, false), symbolId);
        annoExprs.push_back(idConst);
        newAnnos->add(new IR::Annotation("id", annoExprs));
        ext->annotations = newAnnos;
    }
