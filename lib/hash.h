...
template<typename T>
auto fnv1a(const T& obj, typename std::enable_if<std::is_standard_layout<T>::value && !std::is_pointer<T>::value>::type * = nullptr)
...
//returns fnv1a hash sum for object with standard layout
template<typename T>
auto murmur(const T& obj, typename std::enable_if<std::is_standard_layout<T>::value && !std::is_pointer<T>::value>::type * = nullptr)
