...
// Normalize legacy placeholders to positional Abseil conversions. Arguments
// have two representations: native values followed by their stream adapters.
// %N% and %s use the latter to retain Boost's type-independent rendering.
...
void streamArgument(std::ostream &out, const T &arg) {
    ...
    if constexpr (std::is_same_v<Type, Util::SourceInfo>) {
        ...
        if (arg == nullptr) {
            ...
            // Bug reports historically use pointer insertion unless dbprint exists.
            if constexpr (has_dbprint_v<Pointee>)
                arg->dbprint(out);
            else
                out << arg;
        } else if constexpr (std::is_void_v<Pointee> || std::is_function_v<Pointee>) {
            out << arg;
        } else {
    ...
    AbslFormatConvert(const StreamArgument &arg, const absl::FormatConversionSpec &spec,
        ...
        if constexpr (std::is_pointer_v<T> && !std::is_function_v<std::remove_pointer_t<T>>) {
            if (spec.conversion_char() == absl::FormatConversionChar::p)
                out << static_cast<const void *>(arg.value);
            else
                streamArgument<PreferDbprint>(out, arg.value);
        } else {
            streamArgument<PreferDbprint>(out, arg.value);
        }
...
absl::FormatArg nativeArgument(const StreamArgument<PreferDbprint, T> &arg) {
    if constexpr (std::is_arithmetic_v<T>)
        return absl::FormatArg(arg.value);
    else
        return absl::FormatArg(arg);
}

template <bool PreferDbprint, typename Tuple, size_t... Is>
std::string formatTuple(std::string_view format, const Tuple &args, std::index_sequence<Is...>) {
...
/// be mixed. Invalid formats/counts throw std::invalid_argument, as formatting
...
/// integer); %N% and %s retain the legacy stream representation for any type.
...
std::string createFormattedMessageFromTuple(const char *format, const Tuple &args) {
    return detail::formatTuple<PreferDbprint>(format, args,
